  ...
  for (i = 0; i < group->n_mem; i++) {
    if (group->mem[i] == NULL) {
      ...
      if ((!V4L2_TYPE_IS_OUTPUT (obj->type)) &&
          ...
          (!strcmp (obj->videodev, V4L2_DEVICE_PATH_NVDEC_MCCOY) && (is_cuvid == TRUE))))
      {
        if (obj->nvbuf_api_version_new)
            retval = NvBufSurfaceFromFd(expbuf.fd, (void**)(&nvbuf_surf));
#ifndef USE_V4L2_TARGET_NV_X86
        else if (is_cuvid == FALSE) {
            retval = NvBufferGetParams (expbuf.fd, &params);
        }
#endif
        if (retval != 0) {
          g_print ("Failed to get params from fd = %d\n", expbuf.fd);
          data = MAP_FAILED;
        } else {
          if (obj->nvbuf_api_version_new)
            data = nvbuf_surf;
 ...
 /* TODO: This could a possible bug in library */
  ...
  if (V4L2_TYPE_IS_MULTIPLANAR (obj->type) &&
    ...
    if (obj->nvbuf_api_version_new)
      ...
      buffer.m.planes[0].bytesused = NvBufferGetSize();
#endif
  }
#endif
