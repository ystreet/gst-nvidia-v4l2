    ...
    GstBuffer * src)
{
  ...
  if (((!strcmp (pool->obj->videodev, V4L2_DEVICE_PATH_NVDEC)) && (is_cuvid == FALSE)) ||
     ...
     ((!strcmp (pool->obj->videodev, V4L2_DEVICE_PATH_NVENC)) && (!V4L2_TYPE_IS_OUTPUT (pool->obj->type))))
  {
  ...
  if ((!strcmp (pool->obj->videodev, V4L2_DEVICE_PATH_NVENC)) && !V4L2_TYPE_IS_OUTPUT (pool->obj->type))
  {
    ...
    if (pool->obj->nvbuf_api_version_new)
    {
      ...
      if (!nvbuf_surf->surfaceList[0].mappedAddr.addr[0])
        ...
        already_mapped = TRUE;
      if (retn != 0) {
        GST_ERROR_OBJECT (src,"NvBufferMemMap Failed for fd = %d", outmemory->dmafd);
        return FALSE;
      }
    ...
    {
      retn = NvBufferMemMap ((gint)outmemory->dmafd, 0, NvBufferMem_Read_Write, &sBaseAddr);
      if (retn != 0) {
        GST_ERROR_OBJECT (src,"NvBufSurfaceFromFd Failed for fd = %d", outmemory->dmafd);
        return FALSE;
      }
    ...
    if (pool->obj->nvbuf_api_version_new){
      if (already_mapped == FALSE)
        ...
        retn = NvBufferMemUnMap ((gint)outmemory->dmafd, 0, &sBaseAddr);
#endif
    if (retn != 0) {
      GST_ERROR_OBJECT (src,"NvBufferMemUnMap Failed for fd = %d", outmemory->dmafd);
  ...
  if ((!strcmp (pool->obj->videodev, V4L2_DEVICE_PATH_NVENC)) && V4L2_TYPE_IS_OUTPUT (pool->obj->type))
  {
    ...
    if (is_cuvid == FALSE) {
      ...
      if (pool->obj->nvbuf_api_version_new)
      {
          ...
          input_dmabuf_fd = src_buf->surfaceList[0].bufferDesc;
      }
      else
      {
    ...
    GstBuffer * dest, GstBuffer * src)
{
  ...
  if ((!strcmp (pool->obj->videodev, V4L2_DEVICE_PATH_NVENC)) && V4L2_TYPE_IS_OUTPUT (pool->obj->type))
  {
    ...
    if (pool->obj->nvbuf_api_version_new)
    {
      ...
      retn = ExtractFdFromNvBuffer (inmap.data, &dmafd);
      if (retn != 0) {
        GST_ERROR_OBJECT (pool, "could not extract fd, failed to import dmabuf");
        gst_buffer_unmap (src, &inmap);
        return GST_FLOW_ERROR;
      }
    }
